...
TARGET_CHARS="8 * 1024 * 1024"   # 8MB (default: 1MB)
...
echo "Current: CHARS=$current_chars, LINES=$current_lines"

if [ "$current_chars" = "$TARGET_CHARS" ] && [ "$current_lines" = "$TARGET_LINES" ]; then
...
echo "Patched: CHARS=$new_chars, LINES=$new_lines"
